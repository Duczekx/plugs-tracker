...
export default function PartsPage() {
  ...
  const filters = usePartsFilters({ debounceMs: 300 });
  ...
  const loadParts = async (
    ...
    if (nextCategories.length) {
      params.set("cat", serializeCategoryParam(nextCategories));
    }
  ...
  useEffect(() => {
    loadParts(1, filters.query, filters.categories, filters.sort).catch(() => {
      setNotice({ type: "error", message: "Nie udalo sie pobrac danych." });
      setIsPartsLoading(false);
    });
  }, [filters.query, filters.categories, filters.sort]);
  ...
  const handlePageChange = async (nextPage: number) => {
    ...
    await loadParts(nextPage, filters.query, filters.categories, filters.sort);
  };
